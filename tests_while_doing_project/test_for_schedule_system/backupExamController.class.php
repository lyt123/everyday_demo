...
class ExamController extends CourseExcelImportController
{
    ...
    {
        $post_data = $this->reqAdmin()->reqPost(array('code', 'teacher_name'));
        $teacher_id = current(D('Teacher')->getData(array('name' => $post_data['teacher_name']), array('id')));
        $course_id = current(D('Course')->getData(array('code' => $post_data['code'], 'teacher_id' => $teacher_id), array('id')));
        $this->ajaxReturn(D('Course')->update($course_id, array('exam_status' => 7)));
    }
    ...
    {
        $this->reqAdmin();
        $course_data = loadExcel('exam_in_thirteenth_week_course-', 'excel_files/exam/exam_in_thirteenth_week_course/', 2);
...
//dd($course_data);
    ...
    public function clearExamInThirteenthWeekCourseExcel()
    {
        $this->reqAdmin();
        $course_model = D('Course');
        $course_model->startTrans();
        $courses = $course_model->getData(array('exam_status' => 4), array('id'), true);
        foreach ($courses as $course) {
            $result = $course_model->update($course['id'], array('exam_status' => 1));
            $this->checkResult($result, $course_model);
        }
        $course_model->commit();
        $this->ajaxReturn(ajax_ok());
    ...
    public function checkCourseEndInThirteenthWeek()
    {
        $model = D('Course');
        $courses = $model->getData(array('exam_status' => array('NEQ', 7), 'examine_way' => 1), array(), true);
        foreach($courses as $course){
            ...
            $model->update($course['id'], array('exam_status' => 4));
        }
    }
    ...
    public function automaticArrangeExam()
    {
        ...
        if ($post_data['week'] == 13) {
            if ($this->automaticArrangeExamInWeek($course_model, 4, 13)['status'] == 200) {
                $course_model->commit();
                $this->ajaxReturn(ajax_ok());
            }
        }
        if ($post_data['week'] == 19) {
            if (
                $this->automaticArrangeExamForPublicCourseInNineteenthWeek($course_model)['status'] == 200 &&
                $this->automaticArrangeExamInWeek($course_model, 1, 19)['status'] == 200
            ) {
                $course_model->commit();
                $this->ajaxReturn(ajax_ok());
            }
    ...
    public function automaticArrangeExamInWeek($course_model, $exam_status, $exam_week)
    {
...
//$time = time();
        ...
        while ($course_info = $course_model->getData(array('exam_status' => $exam_status, 'examine_way' => 1), array('id', 'teacher_id', 'class_id', 'combine_status', 'name', 'code'))) {
...
//dump($origin_periods);
            //排除教师普教上课时间
            $teacher_existing_class_period = self::filterTeacherExistingClass($course_info['teacher_id'], $exam_week);

            //排除教师其他课程考试时间
            $teacher_other_course_exam_period = self::filterTeacherOtherCourseExam($course_info['teacher_id'], $exam_week, $exam_status + 1);
...
//                dump($teacher_course_period);
            ...
            if ($course_info['combine_status']) {
                $class_other_course_period = self::filterClassOtherCourseExam($course_info['combine_status'], $exam_week, $exam_status + 1);
                if ($exam_week == 13) {
                    $class_course_period = self::filterClassCourseInthirteenthWeek($course_info['combine_status']);
                }
            } else {
                $class_other_course_period = self::filterClassOtherCourseExam($course_info['class_id'], $exam_week, $exam_status + 1);
                if ($exam_week == 13) {
...
//                dd($remain_period);
                if ($course_info['combine_status']) {
                    D('Exam')->addOne(array('code' => $course_info['code'], 'week' => $exam_week, 'period' => $remain_period));
                    ...
                    D('Exam')->addOne(array('code' => $course_info['code'], 'period' => $remain_period, 'week' => $exam_week));
...
//        $post_data = $this->reqAdmin()->reqPost(array('week'));
        $post_data['week'] = 13;
        $model = D('Exam');
        $data = $model->getData(array('week' => $post_data['week']), array('code', 'period'), true);
//        dump($data);
        $excel_data = array();
        $j = 0;
...
//dump($item);continue;
            ...
            for ($i = 0; $i < $sum; $i++) {
                ...
                $excel_data[$j][9] = $model->transferPeriodToDate($item['period'], $post_data['week']);
                $j++;
            }
        }
...
//                dd($excel_data);
        ...
        $model->setWorkExamExcelTitle($objPHPExcel, $post_data['week']);
     ...
     * Auth:lyt123
     */
    public function importExamInWeekExcel()
    {
        $post_data = $this->reqAdmin()->reqPost(array('week'));
        $exam_data = loadExcel('exam_in_' . $post_data['week'] . '_week-', 'excel_files/exam_in_' . $post_data['week'] . '_week/', 2);
        $model = D('Exam');
        $model->startTrans();
//dd($exam_data);
        foreach ($exam_data as $key => $item) {
            ...
            $data['time'] = $item[9];
            $data['place'] = $item[10];
            $data['week'] = $post_data['week'];

            $this->checkResult(D('ExamInWeek')->addOne($data), $model);
        }
        $model->commit();
        $this->ajaxReturn(ajax_ok());
    }
    ...
    /**
     * Des :清空填有考试地点和已修改的考试时间的考试安排表
     * Auth:lyt123
     */
    public function clearExamInWeekExcel()
    {
        $post_data = $this->reqAdmin()->reqPost(array('week'));

        $model = D('ExamInWeek');
        $model->startTrans();
        if ($model->where($post_data)->delete()) {
            $model->commit();
            $this->ajaxReturn(ajax_ok(array(), '清空数据成功'));
        }
        $model->rollback();
    ...
    public function getExamData($data)
    {
        ...
        if ($data['open_class_academy']) {
            ...
            foreach ($select_book_data as $item) {
                ...
                if ($result = D('ExamInWeek')->getData(array('course_id' => $course_id, 'week' => $data['week']))) {
                    $exam_data[] = $result;
                }
            }
        } elseif ($data['have_class_academy']) {
            $academy_id = current(D('Academy')->getData(array('name' => $data['have_class_academy']), array('id')));
            $course_ids = D('Course')->getData(array('academy_id' => $academy_id), array('id'), true);
            D('ExamInWeek')->getExamDataByCourseIds($course_ids, $exam_data, $data['week']);
            ...
            //返回全部数据
            $exam_data = D('ExamInWeek')->getData(array('week' => $data['week']), array(), true);
        }
//dd($exam_data);
...
//            $item['monitor_teacher_name'] = D('Teacher')->transferMultipleTeacherIdToTeacherNames($item['monitor_teacher_id']);
    ...
    {
        $data = $this->reqPost(array(), array('open_class_academy', 'have_class_academy', 'teacher_name', 'class_name', 'code', 'week', 'name'));
    ...
    {
        $post_data = $this->reqAdminAcademy()->reqPost(array('exam_id', 'monitor_teacher_name', 'week'));
//        $data['monitor_teacher_id'] = D('Teacher')->transferMultipleTeacherNameToTeacherIds($post_data['teacher_name']);//凌德全老师在全校教师账号里面没有，所以监考教师暂时不存id了
        $this->ajaxReturn(D('ExamInWeek')->update($post_data['exam_id'], $post_data));
    }
    ...
    {
//        $data = $this->reqPost(array(), array('open_class_academy', 'have_class_academy', 'teacher_name', 'class_name', 'week'));
        $data = ['week' => 19];
//        $data['academy_id'] = 1;
        $model = D('Exam');
        $exam_data = $this->getExamData($data);
...
//        dd($excel_data);
    ...
    public function clearAlreadyExamCourseExcel()
    {
        $this->reqAdmin();
        $model = D('Course');
        $model->startTrans();
        $course_data = $model->getData(array('exam_status' => 7), array('id'), true);

        foreach($course_data as $course){
            $result = $model->update($course['id'], array('exam_status' => 1));
...
//                dump($week_section_ids);
     ...
     */
    public
    function filterTeacherExistingClass($teacher_id, $exam_week)
    {
...
//        dd($existing_class_period);
    ...
    public
    function filterTeacherOtherCourseExam($teacher_id, $exam_week, $exam_status)
    {
        //教师其他有考试的课程的考试时间
    ...
    public
    function filterClassOtherCourseExam($class, $exam_week, $exam_status)
    {
        $period = array();
